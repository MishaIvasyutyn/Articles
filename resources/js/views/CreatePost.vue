<template>
    <div class="px-16 py-8 mt-8">
        <div v-if="error" class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800"
             role="alert">
            <span v-for="error in error.title"   class="font-medium">{{error}}</span>
            <span v-for="error in error.body"   class="font-medium ml-5">{{error}}</span>
        </div>
        <form @submit.prevent="storePost">
            <h1 class=" flex justify-center text-2xl font-bold text-gray-600 mb-4">Create Post</h1>
            <div class="mb-6">
        ...
        storePost() {
            ...
            axios.post('/api/posts', this.form, {
                ...
                if (response.status == 201) {
                    this.form.title = ''
                    this.form.body = ''
                    this.$router.push('/article/' + response.data.post.id)
                }
            }).catch(error => {
                setTimeout(() => {
                    this.loading = false;
